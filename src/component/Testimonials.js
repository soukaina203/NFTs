...
{reviews.map((e)=>{
    return <div className='max-w-sm p-10 pb-6 mx-auto bg-[#2F2963] my-auto duration-300 transform shadow-md cursor-pointer relative'>
    <img src={`nfts/${e.image}.png`} key={e.image} alt="" 
         className='absolute top-0  w-[6rem] h-[6rem] -translate-y-1/2  -translate-x-1/2 rounded-full ml-auto mr-auto' />
    <h3 key={e.image} className='font-bold text-center mt-10 lg:mt-[0.4rem] lg:text-xl'>{e.name}</h3>
    <p key={e.image} className="text-[21px] w-[14rem] mb-4 mx-auto mt-1 text-center">
